...
class TradingApp {
    ...
    updateServerConnectionStatus(connected, lastUpdate) {
        const serverConnectionText = document.getElementById('server-connection-text');
        if (!serverConnectionText) return;
...
// API Status Display Functions (Server-side only)
function updateServerStatus(apiStatus) {
...
function checkApiStatusAndDisplay() {
    fetch("/api/price-source-status")
        .then(response => response.json())
        .then(data => {
            // Server status updated via price source status endpoint
        })
        .catch(error => {
