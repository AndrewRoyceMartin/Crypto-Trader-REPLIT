...
class EntryConfidenceAnalyzer:
    ...
    def _generate_timing_signal(self, confidence_score: float, df: pd.DataFrame, current_price: float) -> str:
        ...
        if confidence_score >= 70:
            ...
            return "CAUTIOUS_BUY"
        elif confidence_score >= 50 or confidence_score >= 40:
            return "WAIT"
        else:
            return "AVOID"
    ...
    def _calculate_intelligent_target_price(self, df: pd.DataFrame, current_price: float, confidence_score: float) -> float:
        ...
        try:
            ...
            (current_price - month_avg) / month_avg * 100 if month_avg > 0 else 0
            ...
            if confidence_score >= 85:
                ...
                if short_momentum > 1:  # Some upward momentum
                    ...
                    self.logger.info("⏳ UNCERTAIN CONDITIONS: 2.5% discount, waiting for clarity")
    ...
    def _fetch_market_data(self, symbol: str, days: int = 30, current_price: float | None = None) -> list[dict]:
    ...
    def _create_fallback_data(self, current_price: float | None = None) -> list[dict]:
