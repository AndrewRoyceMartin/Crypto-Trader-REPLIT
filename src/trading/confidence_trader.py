...
class ConfidenceBasedTrader:
    ...
    def _is_in_cooldown(self, symbol: str) -> bool:
        ...
        cooldown_end = last_purchase.replace(tzinfo=UTC) + timedelta(minutes=self.purchase_cooldown_minutes)
    ...
    def execute_confidence_purchase(self, symbol: str, amount_usd: float, confidence_data: dict[str, Any]) -> bool:
        ...
        try:
            ...
            if order_response and order_response.get('id'):
                ...
                self.last_purchase_times[symbol] = datetime.now(UTC)
...
def get_confidence_trader(config: Config | None = None) -> ConfidenceBasedTrader:
